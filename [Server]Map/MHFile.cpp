...
{
	if( m_bReadMode == MHFILE_PACKEDFILE )
		return (m_Header.FileSize == (DWORD)m_Dfp) ? TRUE : FALSE;
	else if( m_bReadMode == MHFILE_NORMALMODE )
...
{
	if( ( m_Header.dwVersion != (DWORD)(2349162036+m_Header.dwType+m_Header.FileSize) ) && ( m_Header.dwVersion != (DWORD)(20040308+m_Header.dwType+m_Header.FileSize) ))
	{
		Clear();
		MessageBox(
			NULL,
