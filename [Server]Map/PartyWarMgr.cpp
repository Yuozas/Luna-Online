...
{	
	...
	if( pPlayer->IsShowdown() || pPlayer->IsPKMode() )	return FALSE;
...
void CPartyWar::PartyWarSuggestAccept( DWORD dwChannel )
{
	...
	for( int i = 0; i < MAX_PARTY_LISTNUM; ++i )
	{
		...
		Msg1.Member[i].ManaPercent = pPartyInfo1[i].ManaPercent;
		...
		Msg2.Member[i].ManaPercent = pPartyInfo2[i].ManaPercent;
...
void CPartyWarMgr::Msg_PartyWarSuggest( void* pMsg )
{
	...
	// check state
	if( pSender->IsPKMode() || pSender->IsShowdown() )
	...
	if( pReceiverPartyMaster->IsPKMode() || pReceiverPartyMaster->IsShowdown() )
