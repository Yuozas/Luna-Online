...
*/	{
	...
	if(pItemKindInfo->Image2DNum != pInvenItem->wIconIdx)
	{
		goto ITEMBUY_FAILED;
	}
...
BOOL cStreetStallManager::BuyDupItem( CPlayer* pOwner, cStreetStall* pStall, CPlayer* pGuest, STREETSTALL_BUYINFO* pBuyInfo )
{
	...
	if( !	pBuyInfo->ItemInfo.Durability ||
			pBuyInfo->ItemInfo.Durability > MAX_YOUNGYAKITEM_DUPNUM )
	{
		return FALSE;
	}
...
*/	{
	...
	else
	{
		...
		//---KES StreetStall Fix 071020
		//---뒤집히는 경우 방지
		if(LeftDurability > MAX_YOUNGYAKITEM_DUPNUM ) goto ITEMBUY_FAILED;
		...
		*/
		...
		// 071126 KTH --- Add Sealed, RemainSecond
		SSItemInsert( pGuest->GetID(), ItemBase.wIconIdx, wBuyNum, absPosOut, pOwner->GetID(), (WORD)(pInvenItem->ItemParam & ITEM_PARAM_SEAL), ItemBase.nSealed, ItemBase.nRemainSecond );
		/////////////////////////////////////////////////////////////
	}
...
BOOL cStreetStallManager::SellDupItem( CPlayer* pOwner, cStreetStall* pStall, CPlayer* pGuest, STREETSTALL_BUYINFO* pSellInfo )
{
	...
	if( !	pSellInfo->ItemInfo.Durability ||
			pSellInfo->ItemInfo.Durability > MAX_YOUNGYAKITEM_DUPNUM )
	{
		return FALSE;
	}
...
*/	{
	...
	{	//팔려는 갯수가 인벤의 갯수보다 적으면
		...
		//---KES StreetStall Fix 071020
		//---뒤집히는 경우 방지
		if(LeftDurability > MAX_YOUNGYAKITEM_DUPNUM ) goto ITEMSELL_FAILED;
		...
		*/		
		...
		// 071126 KTH --- add Sealed, RemainSecond
		SSItemInsert( pOwner->GetID(), ItemBase.wIconIdx, wSellNum, absPosOut, pGuest->GetID(), (WORD)(pInvenItem->ItemParam & ITEM_PARAM_SEAL), ItemBase.nSealed, ItemBase.nRemainSecond);
		//////////////////////////////////////////////////////////////////////////
...
{
	if (g_csFarmManager.GetFarmZoneID(g_pServerSystem->GetMapNum(), NULL)) return;
	...
	case MP_STREETSTALL_LOCKITEM_SYN:
		{
			...
			for( int i = 0; i < MAX_STREETSTALL_CELLNUM; ++i )
			{
				...
				if( pInfo->sItemBase.Durability == 0 )
					wDurability = 1;
				else
					wDurability = pInfo->sItemBase.Durability;
...
/*			if( pItemKindInfo->ItemKind & eSHOP_ITEM )
			{
				...
				// 교환불가 아이템은 거래 안되도록
				if( pItemKindInfo->ItemKind == eSHOP_ITEM_EQUIP && pItemKindInfo->BuyPrice )
					goto ITEMREGISTFAILEX;
...
*/			{
			...
			if( !pStall->CheckItemDBIdx(pBase.wIconIdx) ) 
			{
				erroflag = 1;
				goto ITEMREGISTFAILEX; // 이미 등록되어 있는 아이템인지...
			}
	...
	case MP_STREETSTALL_BUYITEM_SYN:
		{
			...
			if( pStall->CheckItemDBIdx(msg->ItemInfo.wIconIdx) ) // 등록아이템인지 체크한다.
			{
				MSG_WORD msgNack;
				msgNack.Category	= MP_STREETSTALL;
				msgNack.Protocol	= MP_STREETSTALL_BUYITEM_NACK;
...
/*	if( pItemKindInfo->ItemKind &eSHOP_ITEM )
	{
		pOwnerInventory	= pOwner->GetSlot( eItemTable_ShopInven );
		pGuestInventory	= pGuest->GetSlot( eItemTable_ShopInven );
		Table = eItemTable_ShopInven;
	}
	else
*/	{
		//---KES StreetStall Fix 071020
		pOwnerInventory	= pOwner->GetSlot( eItemTable_Inventory );
		//-----------------------------
		pGuestInventory	= pGuest->GetSlot( eItemTable_Inventory );
	}	
	//
