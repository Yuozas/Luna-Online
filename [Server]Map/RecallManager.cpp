			...
			for( DWORD j = 0 ; j < CHANNELSYSTEM->GetChannelCount() ; ++j )
			{
				...
				if( dwChannelID )
				{
					...
					*/
					...
					if(pList)
					{
						CMonster* pMonster = REGENMGR->RegenObject( dwID, dwSubID, dwChannelID, pList->ObjectKind, pList->MonsterKind, &vRecallPos, /*EVENTRECALL_GROUP_ID*/dwGroupID, DropItemID, dwDropRatio, FALSE, TRUE );
						if(pMonster && dwDieTime)
						{
							pMonster->SetDieTime(dwDieTime + gCurTime);
						}
					}
				}
			}
...
void CRecallManager::ChallengeZoneRecall( UINT wMonsterKind, BYTE bMonsterCount, DWORD dwChannel, VECTOR3* pMonsterPos, WORD wRadius )
{
	...
	for( BYTE i = 0; i < bMonsterCount; ++i )
	{
		if( dwChannel )
		{
			...
			REGENMGR->RegenObject( dwID, dwSubID, dwChannel, eObjectKind_Monster, wMonsterKind, &vRecallPos, CHALLENGERECALL_GROUP_ID, 0, 100, FALSE, TRUE );
		}
	}
}
...
// E µ¥ÀÌÆ® Á¸ Ãß°¡ added by hseos 2007.11.28
