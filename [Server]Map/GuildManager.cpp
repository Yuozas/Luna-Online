	...
	case MP_GUILD_DELETEMEMBER_SYN:
		{
			...
			// 071213 LUJ, 의미없는 코드 player 포인터가 유효한지 체크하는 것으로 충
			// 맵에 접속되어 있지 않은 상태로 요청이 왔는가 체크
	...
	case MP_GUILD_LEVELUP_SYN:
		{
			...
			// DB에 작업 요청: 점수와 퀘스트 수행 여부는 DB에서 비교할 것이다. 동기화 땜시...
			GuildSetLevel(
				guildIndex,
				playerIndex,
...
//		SendNackMsg( pPlayer, MP_GUILD_LEVELUP_NACK, eGuildErr_Err);
//
//	/*어빌리티 삭제 - if(ABILITYMGR->GetAbilityLevel(ABILITYINDEX_LEADERSHIP, pPlayer->GetAbilityGroup()) <= pGuild->GetLevel())
