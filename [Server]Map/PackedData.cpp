...
void CPackedData::Init(int MaxConnectionIndex,void* pData,WORD MsgLen)
{
	...
	if( MsgLen > max )
	{
		...
		if( fp )
		{ 
			fprintf(fp, "\n%s%d (%02d:%02d:%02d)\n", "CPackedData::Init() MAX_MsgLen = ", max, time.wHour, time.wMinute, time.wSecond );
			fclose(fp);
		}
	}
	//============
...
//------------------------------	
		if( TotalDataLen > max )
		{
			...
			if( fp )
			{ 
				fprintf(fp, "\n%s%d [%d] (%02d:%02d:%02d)\n", "CPackedData::Send() MAX_TotalDataLen = ", max, m_SendData.wReceiverNum, time.wHour, time.wMinute, time.wSecond );
				fprintf(fp, "LastMessage : Category - %d, Protocol - %d\n", ((MSGBASE*)m_SendData.Data)->Category, ((MSGBASE*)m_SendData.Data)->Protocol );
				fclose(fp);
			}
		}
		//============        
...
void CPackedData::SendToMapServer(DWORD AgentNum,WORD ToMapNum,MSGBASE* pMsg,WORD MsgLen)
{
	...
	if( MsgLen > max )
	{
		...
		if( fp )
		{ 
			fprintf(fp, "\n%s%d (%02d:%02d:%02d)\n", "CPackedData::SendToMapServer() MAX_MsgLen = ", max, time.wHour, time.wMinute, time.wSecond );
			fclose(fp);
		}
	}
	//============
...
void CPackedData::SendToBroadCastMapServer(MSGBASE* pMsg, WORD MsgLen)
{
	...
	if( MsgLen > max )
	{
		...
		if( fp )
		{ 
			fprintf(fp, "\n%s%d (%02d:%02d:%02d)\n", "CPackedData::SendToBroadCastMapServer() MAX_MsgLen = ", max, time.wHour, time.wMinute, time.wSecond );
			fclose(fp);
		}
	}
	//============
