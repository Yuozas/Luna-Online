...
public:
	...
	CMonster* RegenObject(DWORD dwID, DWORD dwSubID, DWORD dwGridID, WORD wObjectKind, WORD wMonsterKind, VECTOR3 * vPos, DWORD dwGroupID, DWORD DropItemID = 0, DWORD dwDropRatio = 100 ,BOOL bRandomPos = TRUE, BOOL bEventRegen = FALSE);
	// 080616 LUJ, 함정을 생성한다
	BOOL RegenTrap( CObject*, DWORD monsterKind );
};
