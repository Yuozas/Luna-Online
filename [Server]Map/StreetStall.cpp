...
BOOL cStreetStall::FillCell( ITEMBASE* pBase, DWORD money, BOOL bLock, DWORD Volume, WORD wAbsPosition )
{
	...
	if(m_wStallKind == eSK_SELL)
	{
		...
		//// 081013 KTH -- 상점에 등록한 아이템의 판매 가격이 최대 소지금액 보다 크지 않게 해준다.
		//for(int n = 0; n < MAX_STREETSTALL_CELLNUM; ++n )
		//{
		//	if( !m_sArticles[n].bFill )
		//		continue;
		//	// 081013 KTH -- 판매 아이템의 돈의 량
		//	dwTotalMoney = m_sArticles[n].dwMoney * m_sArticles[n].wVolume;
		//}

		//if( dwTotalMoney > MAX_INVENTORY_MONEY )
		//	return FALSE;
		//else if( dwTotalMoney+m_pOwner->GetMoney() > MAX_INVENTORY_MONEY )
		//	return FALSE;
	}
...
void cStreetStall::GetStreetStallInfo( STREETSTALL_INFO& stall )
{
	...
	if( OptionNum )
	{
		stall.AddableInfo.AddInfo( CAddableInfoList::ItemOption, sizeof( OptionInfo[ 0 ] )*OptionNum, OptionInfo );
