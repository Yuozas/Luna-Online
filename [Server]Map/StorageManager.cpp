	...
	case MP_STORAGE_PUTIN_MONEY_SYN:
		{
			...
			// 080608 LYW --- StorageManager : 창고 소환 아이템이 생기면서, 창고 npc 앞에서가 아닌, 필드 
			// 어디에서도 창고를 사용할 수 있게 되었다. 때문에, 입/출금 시 거리 체크를 하는 CheckHack 함수 호출을 삭제한다.
			//if( !CheckHack( pPlayer ) )	return;
...
void CStorageManager::BuyStorageSyn(CPlayer* pPlayer)
{
	...
	{
		if(pPlayer->GetStorageNum() == 0) //처음 샀으면 그냥 초기화
		{
