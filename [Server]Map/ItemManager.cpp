...
//	MHERROR->OutputFile("itemtable.txt", pFromSlotD->_ToStringSlot());
	...
	// 071126 KTH --- 봉인 여부에 따라 움직일수 있는지 체크 한다.
/*	const ITEMBASE* pItemBase	= GetItemInfoAbsIn( pPlayer, FromPos );
	ITEM_INFO* pItemInfo		= GetUseItemInfo( pPlayer, FromPos, FromItemIdx );

	switch( pToSlot )
	{
	case eItemTable_Weared:		// 봉인 되어 있는 아이템은 착용이 불가능하다.
		if( pItemBase->nSealed == eITEM_TYPE_SEAL )
			return 11;

	case eItemTable_Storage:	// 봉인이 풀리고 플레이타임으로 되어있는 아이템은 창고에 저장 불가능.
		if( pItemBase->nSealed == eITEM_TYPE_UNSEAL && pItemInfo->nTimeKind == eKIND_PLAYTIME )
			return 11;

	case eItemTable_Shop:		// 샵 창고안에 아이템을 넣을수 없다.
		return 11;

	case eItemTable_StreetStall:// 봉인이 풀린 아이템은 상점에 등록할 수 없다.
...
/*	if( pToSlot == pPlayer->GetSlot(eItemTable_Shop) )
	...
	// 아바타아이템일 경우 같은 아이템이 사용중이면 이동불가
	/*if( pToSlot == pPlayer->GetSlot(eItemTable_ShopInven) )
	{
		ITEM_INFO* pItemInfo = GetItemInfo( FromItemIdx );
		if(!pItemInfo)		return 11;

		if( pItemInfo->ItemKind == eSHOP_ITEM_MAKEUP || pItemInfo->ItemKind == eSHOP_ITEM_DECORATION )
		{
			if( pPlayer->GetShopItemManager()->GetUsingItemInfo( FromItemIdx ) )
	...
	{
		if( (pItem->nSealed == eITEM_TYPE_UNSEAL && pInfo->nTimeKind == eKIND_PLAYTIME) || pInfo->Deposit == 0 )
	...
	// 071126 KTH --- UnSeal 상태의 아이템은 상점에 등록할 수 없다.
	if( pToSlot == pPlayer->GetSlot(eItemTable_StreetStall) )
	{
		if( pItem->nSealed == eITEM_TYPE_UNSEAL )
			return eItemUseErr_Move;
		// 080204 KTH -- 교환 불가 아이템은 상점에 등록할 수 없다.
		if( !pInfo->Trade )
			return eItemUseErr_Move;
	}
	if( pToSlot == pPlayer->GetSlot(eItemTable_MunpaWarehouse) )
	{
		if( pItem->nSealed == eITEM_TYPE_UNSEAL )
			return eItemUseErr_Move;
		// 080204 KTH -- 교환 불가 아이템은 길드 창고에 넣을수 없다.
		if( !pInfo->Trade )
			return eItemUseErr_Move;
	}

	/*if( pFromSlot == pPlayer->GetSlot(eItemTable_MunpaWarehouse) )
	{
		if( pItem->nSealed == eITEM_TYPE_UNSEAL )
			return eItemUseErr_Move;
	}*/
	// 071127 KTH --- 이동할 곳의 장소가 비어 있지 않고 이동을 시작하는 곳이 창고이면 옴기려는 아이템의 상태를 가져와 봉인이 풀려있으면서 게임시간제 아이템이면 실패로 간주 한다.
...
/*	if( pInfo && pInfo->ItemType == 11 )
	if( pInfo->ItemKind != eSHOP_ITEM_MAKEUP && pInfo->ItemKind != eSHOP_ITEM_DECORATION && pInfo->ItemKind != eSHOP_ITEM_EQUIP && pInfo->ItemKind != eSHOP_ITEM_PET && pInfo->ItemKind != eSHOP_ITEM_PET_EQUIP)
	{
...
/*	const ITEMBASE* pToItem = pToSlot->GetItemInfoAbs(ToPos);
	if( pToItem )
	{
		...
		if( pToInfo->ItemKind != eSHOP_ITEM_MAKEUP && pToInfo->ItemKind != eSHOP_ITEM_DECORATION && pToInfo->ItemKind != eSHOP_ITEM_EQUIP && pToInfo->ItemKind != eSHOP_ITEM_PET && pToInfo->ItemKind != eSHOP_ITEM_PET_EQUIP )
		{
...
//		SWAPVALUE(DelFromItem.nSealed, DelToItem.nSealed);
//		SWAPVALUE(DelFromItem.nRemainSecond, DelToItem.nRemainSecond);
...
/*	if( TP_SHOPITEM_START <= ToPos && ToPos < TP_SHOPITEM_END )
	{
		// ShopItemUseInfo에 UserIdx를 업데이트 해준다.
		// pPlayer->GetUserID();
	}
	if( TP_SHOPINVEN_START <= ToPos && ToPos < TP_SHOPINVEN_END )
	{
		// ShopItemUseInfo에 CharacterIdx를 업데이트해준다.
		// pPlayer->GetID();
	}*/
