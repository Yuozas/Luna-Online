...
*/				pInventory = (CInventorySlot*)pPlayer1->GetSlot( eItemTable_Inventory );
			...
			// 071125 KTH --- 봉인이 풀린 아이템은 교환이 불가능하다. 080204 KTH -- 교환 불가능한 아이템도 교환이 불가능하다.
	...
	case MP_EXCHANGE_INPUTMONEY_SYN:
		{
			...
			//KES
			if( pRoom->IsLocked( pPlayer1->GetExchangeContainer()->nMyIndex ) || pRoom->IsLocked( pPlayer2->GetExchangeContainer()->nMyIndex )
				|| pRoom->GetExchangeMoney( pPlayer1->GetExchangeContainer()->nMyIndex ) > 0 )  //한쪽이라도 락을 했거나, 돈을 한번이라도 입금했을 경우
			{
				MSGBASE NackMsg;
...
//---KES 071205 사기 방지를 위해 락이 안풀리도록 수정
/*
