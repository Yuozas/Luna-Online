...
*/
	...
	//char* CryptoName = pDesc->pszFileName;
	//for(int i=0;i<strlen(CryptoName);i++)
	//		CryptoName[i] ^= 9;
...
BOOL CPackFile::SetPakVersion( DWORD	PakVersion )
{
	for(int i=0; i<m_nVersionCount; i++)
	{	
		if(dwVersion[i] == PakVersion)
		{
...
BOOL CPackFile::AESEncryptData(FILE* fp, char* pFileName, char* pData, DWORD dwDataSize)
{
...
BOOL CPackFile::AESDecryptData(FILE* fp, char* pData, DWORD dwDataSize, DWORD dwType)
{
	if(fp == NULL)		{	return FALSE;	}
	if(pData == NULL)	{	return FALSE;	}
