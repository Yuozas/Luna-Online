...
// -------------------------------------------------------------------------------------------------------------------------------------
//  SRV_ResetRegistResident Method																						 주민등록 초기화
//	080215 KTH
BOOL CSHResidentRegistManager::SRV_ResetRegistResident(CPlayer* pPlayer)
{
	...
	// 맵서버의 데이트 매칭 정보와 주민등록 정보를 초기화한다.
	...
	// 에이전트에 정보를 보내주어야 하는대 리스 하면 상관 없다 ;;
#endif
	return TRUE;
}

// -------------------------------------------------------------------------------------------------------------------------------------
//  SRV_UpdateIntroduction Method																				  자기소개 업데이트 처리
//
	...
	case MP_RESIDENTREGIST_REGIST:
		{
			...
			if (HERO->GetID() == pPacket->dwObjectID)
			{
				...
				//CHATMGR->AddMsg( CTC_SYSMSG, "주민등록증을 발급 받았습니다." );
	...
	case MP_RESIDENTREGIST_UPDATE_INTRODUCTION:
		{
			...
			// notice to chatting dialog.
			//CHATMGR->AddMsg( CTC_SYSMSG, "소개가 변경되었습니다." );
	...
	case MP_RESIDENTREGIST_DATEMATCHING_REQUEST_CHAT:
		{
			...
			case DATE_MATCHING_CHAT_RESULT_LOGOFF:
				{
					//WINDOWMGR->MsgBox( MBI_MATCHAT_CANCEL, MBT_CANCEL, "상대방이 현재 접속 중이 아닙니다." );
...
*/
					//WINDOWMGR->MsgBox( MBI_MATCHAT_CANCEL, MBT_CANCEL, "상대방이 현재 대화 준비 중입니다. 잠시 후 다시 신청해 주세요." );
			...
			case DATE_MATCHING_CHAT_RESULT_REQUEST_AND_WAIT:
				{
					...
					//WINDOWMGR->MsgBox( MBI_MATCHAT_CANCEL, MBT_CANCEL, "상대에게 대화를 신청했습니다. 수락할 때까지 잠시만 기다려 주세요." );
			...
			case DATE_MATCHING_CHAT_RESULT_TIMEOUT:
				{
					...
					if( pMsgBox )
					{
						...
						//WINDOWMGR->MsgBox( MBI_MATCHAT_TIMEOVER, MBT_OK, "초대에 응답이 없어 초대 신청을 취소합니다." );
					...
					if( pMsgBox )
					{
						...
						//WINDOWMGR->MsgBox( MBI_MATCHAT_TIMEOVER, MBT_OK, "초대에 응하지 않았습니다." );
			...
			case DATE_MATCHING_CHAT_RESULT_REFUSE:
				{
					...
					//WINDOWMGR->MsgBox( MBI_MATCHAT_REJECT, MBT_OK, "상대방이 대화 신청을 거절했습니다." );
			...
			case DATE_MATCHING_CHAT_RESULT_END_CHAT:
				{
					...
					if( pDlg )
					{
						...
						{
							//WINDOWMGR->MsgBox( MBI_MATCHAT_STOP, MBT_OK, "대화가 종료되었습니다." );
