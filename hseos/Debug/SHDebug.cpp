...
VOID CSHDebug::ParseTextInput(char* pszText, char* pCmd)
{
	...
	if (stricmp(pCmd, "FARM_BUY_") == 0)
	{
		...
		g_csFarmManager.CLI_RequestBuyFarmToSrv((CSHFarmZone::FARM_ZONE)nFarmZone, nFarmIndex);
	}
	...
	if (stricmp(pCmd, "FARM_UPGRADE") == 0)
	{
		...
		g_csFarmManager.CLI_RequestUpgradeFarm((CSHFarmZone::FARM_ZONE)nFarmZone, nFarmIndex, nObjKind);
	}
...
//		g_csResidentRegistManager.CLI_RequestDateMatchingChat(nID);
	...
	if (stricmp(pCmd, "DATEZONELIST") == 0)
	{
 	...
 	if (stricmp(pCmd, "MARRIAGE") == 0)
	{
//		g_csMarriageManager.CLI_RequestMarriage(TRUE);
	}

	if (stricmp(pCmd, "MARRIAGE_NICKNAME") == 0)
	{
		char szName[256] = {0};

		sscanf(pszText, "%s %s", pCmd, szName);

//		g_csMarriageManager.CLI_RequestNickname(szName);
	}
#endif
}
