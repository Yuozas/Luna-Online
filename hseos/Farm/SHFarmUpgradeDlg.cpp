...
void CSHFarmUpgradeDlg::Render()
{
	...
	case CSHFarm::FARM_EVENT_GARDEN:
		nGrade = g_csFarmManager.GetFarmZone(m_nFarmZone)->GetFarm(m_nFarmID)->GetGarden(0)->GetGrade();
		break;
	case CSHFarm::FARM_EVENT_FENCE:
		nGrade = g_csFarmManager.GetFarmZone(m_nFarmZone)->GetFarm(m_nFarmID)->GetFence(0)->GetGrade();
		break;
	case CSHFarm::FARM_EVENT_HOUSE:
		nGrade = g_csFarmManager.GetFarmZone(m_nFarmZone)->GetFarm(m_nFarmID)->GetHouse(0)->GetGrade();
		break;
	case CSHFarm::FARM_EVENT_WAREHOUSE:
		nGrade = g_csFarmManager.GetFarmZone(m_nFarmZone)->GetFarm(m_nFarmID)->GetWarehouse(0)->GetGrade();
		break;
	case CSHFarm::FARM_EVENT_ANIMALCAGE:
		nGrade = g_csFarmManager.GetFarmZone(m_nFarmZone)->GetFarm(m_nFarmID)->GetAnimalCage(0)->GetGrade();
		break;
	}
