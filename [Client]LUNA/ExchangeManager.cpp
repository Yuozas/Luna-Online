...
void CExchangeManager::NetworkMsgParse( BYTE Protocol, void* pMsg )
{
	...
	case MP_EXCHANGE_UNLOCK:				Exchange_Unlock() ;						break;
...
void CExchangeManager::Exchange_Additem( void* pMsg )
{
	...
	
	{
		...
		/*const ITEM_OPTION* pOption = ITEMMGR->GetOption( msg->ItemInfo.wIconIdx, msg->ItemInfo.Durability ) ;

		ITEMMGR->AddToolTip( pItem, pOption, msg->ItemInfo.dwDBIdx );*/
	}
...
void CExchangeManager::Exchange_Unlock()
{
	return;

	GAMEIN->GetExchangeDialog()->OpLock( FALSE );
	CHATMGR->AddMsg( CTC_SYSMSG, CHATMGR->GetChatMsg(452) );
