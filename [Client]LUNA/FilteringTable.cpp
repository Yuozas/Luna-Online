...
BOOL CFilteringTable::FilterChat( char* pChat )
{
	...
	if( checkChar == checkCommand )	// 첫 글자가 어떤 명령을 실행하는 /와 같으면, 
	{
		char desc[128] = {0, } ;
		char* tempStr1 ;
...
BOOL CFilteringTable::IsUsableName( const char* pName, BOOL bUseByte )
{
...
//#endif
		...
		// desc_hseos_문자필터링01
		// S 문자필터링 추가 added by hseos 2007.06.25
		// ..이 함수의 리턴전에 반드시 SetExceptionInvalidCharIncludeType(FET_NONE); 호출해야 함!!!!!
		// if( bOk == FALSE )
		//	return TRUE;
		if( bOk == FALSE )
		{
