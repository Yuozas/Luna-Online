...
void cItemPopupDlg::Linking() 											
{
	...
	mTextPosX = mpTextArea->GetAbsX();
	mTextPosY = mpTextArea->GetAbsY();;
}
...
void cItemPopupDlg::SetActive( BOOL val )
{
	if( val )
	{
		...
		if( nTextLine < 2 )
		{
			...
			mpBottom->SetAbsY( mpMiddle->GetAbsY() );		
		}
		else
		{
			cImageScale scale;
			...
			mpBottom->SetAbsY( mpMiddle->GetAbsY() + ( mpMiddle->GetHeight() * ( nTextLine - 1 ) ) );		
		}
	}
