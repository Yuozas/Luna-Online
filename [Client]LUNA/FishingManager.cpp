...
void CFishingManager::Fishing_Mission_Update(void* pMsg)
{
	...
	DWORD dwMissionCode = pmsg->dwItem1*10000 + pmsg->dwItem2*100 + pmsg->dwItem3;

	DWORD dwMission[MAX_FISHING_MISSION];
...
void CFishingManager::Fishing_Exp_Ack(void* pMsg)
{
	...
	sprintf( szValue, "%d/%d (%s)", m_dwFishingExp, dwMaxExp, szLevel);
...
void CFishingManager::Process()
{
	...
	else if(m_bPulling && m_dwFishingStartTime+FISHINGTIME_STARTPULLING+m_dwProcessTime <= gCurTime)
	{
		...
		msg.wPerpectCnt = pFishingDlg->GetPointPerpect();
		msg.wGoodCnt = pFishingDlg->GetPointGood();
		msg.wGreatCnt = pFishingDlg->GetPointGreat();
		msg.wMissCnt = pFishingDlg->GetPointMiss();
