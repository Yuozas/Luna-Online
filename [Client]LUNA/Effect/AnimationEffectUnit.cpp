...
void CAnimationEffectUnit::DoOn(EFFECTPARAM* pParam,TARGETSET* pObject)
{
	...
	if(pDesc->m_CustumMotion)
		pObject->pTarget->GetEngineObject()->ChangeCustumMotion(pDesc->m_MotionNum,
								pDesc->m_StartFrame,pDesc->m_EndFrame,pDesc->m_bRepeat,pParam->m_AnimateRate);
	else
		pObject->pTarget->ChangeMotion(pDesc->m_MotionNum,pDesc->m_bRepeat,pParam->m_AnimateRate);
}
