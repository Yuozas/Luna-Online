...
void CPartyWar::Init()
{
	...
	// 070202 LYW --- CPartyWar : Modified limited area.
...
{
	ZeroMemory( &m_EnemyPartyInfo,	sizeof( m_EnemyPartyInfo ) );
	ZeroMemory( m_MemberIdx,		sizeof( m_MemberIdx ) );
...
{
	if( m_nWarState == ePartyWar_Fight )
	{
		const DWORD dwEnemyId = pTarget->GetID();

		for( int i = 0; i < MAX_PARTY_LISTNUM; ++i )
		{
			if( m_EnemyPartyInfo.Member[i].bLogged			&&
				m_EnemyPartyInfo.Member[i].dwMemberID == dwEnemyId )
			{
				return TRUE;
			}
		}
...
CPlayer* CPartyWar::CanPartyWarSuggest()
{
	...
	if( HERO->IsPKMode() || SHOWDOWNMGR->IsShowdown() )
	{
...
void CPartyWar::PartyWarReady( MSG_BATTLE_SHOWDOWN_CREATESTAGE* pMsg )
{
