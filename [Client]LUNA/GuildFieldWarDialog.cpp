...
{
	const DWORD unableColor = RGB( 100, 100, 100 );
...
void CGFWarDeclareDlg::Render()
{
	...
	if( m_bRender )
	{
		...
		{
			// 070209 LYW --- CGFWarDeclareDlg : Modified render part.
			if( IsMovedWnd() )
			{
				//m_vScale.x = m_pMoneyEdit->GetWidth();
				//m_vScale.y = m_pMoneyEdit->GetHeight();
			...
			if( !m_pMoneyCheck->IsChecked() )
			{
				...
				//if( IsMovedWnd() )
				//{
					//m_vPos.x = this->m_absPos.x + 65;	
					//m_vPos.y = this->m_absPos.y + 269;

					
				//}
				//m_LockImage.RenderSprite( &m_vScale, NULL, 0.0f, &m_vPos,
				//						  RGBA_MERGE(m_dwImageRGB, m_alpha * m_dwOptionAlpha / 100) );
			}
			else
			{
				m_pMoneyEdit->SetImageRGB( unableColor );
			}
...
void CGFWarResultDlg::ShowDeclare( const MSG_GUILD_LIST::Data& data )
{
	...
	if( data.UnionIdx )
	{
		//sprintf( line, "명    칭 :  %s (%s 동맹)", data.GuildName, data.mUnionName );
	...
	//sprintf( line, "선 포 금 :  %s골드", AddComma( data.MarkName ) );
	...
	// 자신의 정보를 표시한다
	{
		{			
			...
			if( *allianceName )
			{
				//sprintf( line, "명    칭 :  %s (%s 동맹)", GUILDMGR->GetGuildName(), allianceName );
