...
void CFishingDialog::Render()
{
	...
	if(0<m_dwMissionStartTime && 0<m_dwMissionProcessTime)
	{
		...
		if(m_nMissionTime != nTime && nTime < 10000)
		{
			...
			char szValue[50];
			sprintf( szValue, "%d초", m_nMissionTime);
...
void CFishingDialog::ChangeBaitItem(CItem* pItem)
{
	...
	if(ITEM_KIND_FISHING_BAIT == info->SupplyType)
	{
		...
		if(pPrevItem)
		{
			CItem* pLinkItem = (CItem*)m_ItemBait.GetLinkItem();
			if(pLinkItem)
				GAMEIN->GetInventoryDialog()->SetMultipleItemLock(pLinkItem->GetItemIdx(), FALSE);
...
void CFishingDialog::UpdateBaitItem()
{
	...
	if(pLinkItem)
	{
		 pItem = GAMEIN->GetInventoryDialog()->GetItemForIdx(pLinkItem->GetItemIdx());
	}
	SetBaitItem(pItem);
...
{
	if(NULL == m_ItemBait.GetLinkItem())
		return;
	...
	if(pLinkItem)
	{
		dur	= (WORD)(GAMEIN->GetInventoryDialog()->GetTotalItemDurability(pLinkItem->GetItemIdx()));
	}
...
void CFishingDialog::Fishing_Start()
{
	...
	if(pLinkItem)
	{
		CItem* pItem = GAMEIN->GetInventoryDialog()->GetItemForIdx(pLinkItem->GetItemIdx());
		if(pItem)
		{
