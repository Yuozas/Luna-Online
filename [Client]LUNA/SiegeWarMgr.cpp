...
// 080411 LYW --- SiegeWarMgr : 채팅방 매니저 헤더 호출.
...
{
	CMHFile file;
	if( !file.Init( "System/Resource/SiegeWarMapInfo.bin", "rb" ) )
		return;
...
{
	CMHFile file;
	if( !file.Init( "System/Resource/CastleGateList.bin", "rb" ) )
		return;
	...
	case MP_SIEGEWAR_MOVEIN_ACK:
		{
			...
			// 080411 LYW --- SiegeWarMgr : 채팅방에서 채팅중이었다면, 채팅내용을 저장한다.
	...
	case MP_SIEGEWAR_BATTLEJOIN_ACK:
		{
			...
			GAMEIN->GetSWTimeDlg()->SetGuildName( pmsg->GuildName );
