...
CItemShopDialog::~CItemShopDialog()
{
	// 071213 LYW --- CItemShopDialog : 인덱스 제네레이터 해제 처리 추가.
...
void CItemShopDialog::SetItemInfo(SEND_SHOPITEM_INFO* pItemInfo)
{
	...
	for(int i=0; i<SLOT_SHOPITEM_IMSI; i++)
	{
		...
		CItem* pItem = m_pItemGridDialog->GetItemForPos( TP_SHOPITEM_START + i );
...
//		int num = GetTabNumber(absPos);
		return (CItem *)m_pItemGridDialog->GetIconForIdx( absPos - TP_SHOPITEM_START );
