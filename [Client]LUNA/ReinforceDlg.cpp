...
BOOL CReinforceDlg::FakeMoveIcon(LONG x, LONG y, cIcon* icon)
{
	...
	else if( info->Improvement )
	{
		...
		if( mProtectionItem.dwDBIdx )
		{
			...
			{
				const EnchantScript* enchantScript = ITEMMGR->GetEnchantScript( option.mEnchant.mIndex );

				if( enchantScript )
				{
					ITEMMGR->AddEnchantValue( option, enchantScript->mAbility, -1 * info->EnchantDeterm * option.mEnchant.mLevel );
				}
			}
...
void CReinforceDlg::AddMaterial( CItem& item )
{
	...
	const ReinforceScript* script = ITEMMGR->GetReinforceScript( item.GetItemIdx() );
	...
	if( mProtectionItem.dwDBIdx )
	{
		...
		// 080228 LUJ, 보호 상태일 경우 각각의 강화 옵션은 현재 위치한 보석이 강화시키는 속성과 일치해야함
		{
			...
			{
				ITEM_OPTION				option			= ITEMMGR->GetOption( targetItem->GetItemBaseInfo() );
				const EnchantScript*	enchantScript	= ITEMMGR->GetEnchantScript( option.mEnchant.mIndex );
	...
	// 이미 강화 옵션이 있는 경우 메시지창을 통해 경고를 표시하도록 한다.
	{
				...
				item->Durability )
		{
			...
			if( enchantScript )
			{
				...
				ITEMMGR->AddEnchantValue( dummyOption, enchantScript->mAbility, -1 * info->EnchantDeterm * option.mEnchant.mLevel );
			}
...
void CReinforceDlg::Send()
{
	...
	// 080228 LUJ, 보호되는 경우 해당 상태의 메시지 출력을 위해 옵션을 저장해둔다
	{
		...
		const EnchantScript*	enchantScript	= ITEMMGR->GetEnchantScript( mProtectedOption.mEnchant.mIndex );
		const ITEM_INFO*		info			= ITEMMGR->GetItemInfo( mMessage.mSourceItemIndex );
		
		// 080228 LUJ, 인챈트된 값이 있을 경우 빼줘야 한다
		if( info &&
			enchantScript )
		{
			ITEMMGR->AddEnchantValue( mProtectedOption, enchantScript->mAbility, -1 * info->EnchantDeterm * mProtectedOption.mEnchant.mLevel );
		}
	}
