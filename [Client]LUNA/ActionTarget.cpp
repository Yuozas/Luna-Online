...
void CActionTarget::SetTargetObjectsInArea(CHero* pHero,CSkillArea* pSkillArea,WORD PARAM)
{
	// Hero 인자로 받는 것은 문파 정보라든가 기타 등등 적과의 구별을 위해서 이다.
	// 중심 좌표는 이미 셋팅 되어져 있다.
	OBJECTMGR->GetTargetInArea(pHero,pSkillArea,&m_ObjectArray,IS_PK,PARAM);
}
