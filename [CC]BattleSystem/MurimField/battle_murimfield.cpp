...
void CBattle_MurimField::OnCreate( BATTLE_INFO_BASE* pCreateInfo, CBattleTeam* pTeam1, CBattleTeam* pTeam2 )
{
	...
	// 070202 LYW --- CBattle_MurimField : Modified limited area.
	//m_vImagePos.x = (1024 - 128) / 2;			//128 width
	DISPLAY_INFO	dispInfo ;
	GAMERESRCMNGR->GetDispInfo( &dispInfo ) ;
	m_vImagePos.x = (float)((dispInfo.dwWidth - 128) / 2);			//128 width
	// 070202 LYW --- End.
	m_vImagePos.y = 32;
...
void CBattle_MurimField::OnTeamMemberRevive(int Team,CObject* pReviver)
{
	...
	DWORD MaxMana = pReviver->GetMaxMana();
	pReviver->SetLife((DWORD)(MaxLife));
	pReviver->SetMana((DWORD)(MaxMana));
...
void CBattle_MurimField::Victory( int WinnerTeamNum, int LoserTeamNum )
{
	...
	*/
	DISPLAY_INFO	dispInfo ;
	GAMERESRCMNGR->GetDispInfo( &dispInfo ) ;
	m_vImagePos.x = (dispInfo.dwWidth - 128 * m_vImageScale.x ) / 2;	//128 = width
	m_vImagePos.y = ( dispInfo.dwHeight - 42 * m_vImageScale.y ) / 2;		//42 = height
	// 070202 LYW --- End.
