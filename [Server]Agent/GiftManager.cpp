...
cGiftManager::cGiftManager(void)
{
	...
	mpGiftEventPool = new CMemoryPoolTempl<cGiftEvent>;
...
cGiftManager::~cGiftManager(void)
{
	...
	while( pInfo = mEventInfoTable.GetData() )
	{
		delete pInfo;
	}
	...
	while( pEvent = mGiftEventTable.GetData() )
	{
...
void cGiftManager::Init()
{
	...
	while(1)
	{
		...
		{
			for( int count = 0; count < mStartEventCount; count++ )
			{
