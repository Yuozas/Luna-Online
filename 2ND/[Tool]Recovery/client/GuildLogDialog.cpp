...
IMPLEMENT_DYNAMIC(CGuildLogDialog, CDialog)
CGuildLogDialog::CGuildLogDialog( CclientApp& application, CWnd* pParent /*=NULL*/)
	: CDialog(CGuildLogDialog::IDD, pParent)
...
CGuildLogDialog::~CGuildLogDialog()
{
}

void CGuildLogDialog::OnFinalRelease()
{
...
void CGuildLogDialog::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_GUILD_LOG_RESULT_PROGRESS, mResultProgressCtrl);
	DDX_Control(pDX, IDC_GUILD_LOG_RESULT_STATIC, mResultStatic);
	DDX_Control(pDX, IDC_GUILD_LOG_RESULT_LIST, mResultListCtrl);
	DDX_Control(pDX, IDC_GUILD_LOG_FIND_BUTTON, mFindButton);
	DDX_Control(pDX, IDC_GUILD_LOG_STOP_BUTTON, mStopButton);
	DDX_Control(pDX, IDC_GUILD_LOG_BEGIN_DATE_CTRL, mBeginDateCtrl);
	DDX_Control(pDX, IDC_GUILD_LOG_BEGIN_TIME_CTRL, mBeginTimeCtrl);
	DDX_Control(pDX, IDC_GUILD_LOG_END_DATE_CTRL, mEndDateCtrl);
	DDX_Control(pDX, IDC_GUILD_LOG_END_TIME_CTRL, mEndTimeCtrl);
}


BEGIN_MESSAGE_MAP(CGuildLogDialog, CDialog)
	ON_BN_CLICKED(IDC_GUILD_LOG_FIND_BUTTON, OnBnClickedGuildLogFindButton)
	ON_BN_CLICKED(IDC_GUILD_LOG_STOP_BUTTON, OnBnClickedGuildLogStopButton)
	ON_BN_CLICKED(IDC_GUILD_LOG_EXCEL_BUTTON, OnBnClickedGuildLogExcelButton)
	ON_BN_CLICKED(IDC_GUILD_LOG_CLIPBOARD_BUTTON, OnBnClickedGuildLogClipboardButton)
END_MESSAGE_MAP()

BEGIN_DISPATCH_MAP(CGuildLogDialog, CDialog)
END_DISPATCH_MAP()

// 참고: IID_IGuildLogDialog에 대한 지원을 추가하여
// VBA에서 형식 안정적인 바인딩을 지원합니다. 
// 이 IID는 .IDL 파일에 있는 dispinterface의 GUID와 일치해야 합니다.

// {C69FCF00-031F-4BBB-BB67-070BB1EFA5C5}
static const IID IID_IGuildLogDialog =
{ 0xC69FCF00, 0x31F, 0x4BBB, { 0xBB, 0x67, 0x7, 0xB, 0xB1, 0xEF, 0xA5, 0xC5 } };

BEGIN_INTERFACE_MAP(CGuildLogDialog, CDialog)
	INTERFACE_PART(CGuildLogDialog, IID_IGuildLogDialog, Dispatch)
END_INTERFACE_MAP()


// CGuildLogDialog 메시지 처리기입니다.

void CGuildLogDialog::OnBnClickedGuildLogFindButton()
{
...

	{
		...
		_stprintf( message.mBeginDate, "%02d%02d%02d %02d:%02d:%02d",
		...
		_stprintf( message.mEndDate, "%02d%02d%02d %02d:%02d:%02d",
			max( 0, date.GetYear() - 2000 ),
			date.GetMonth(),
			date.GetDay(),
			time.GetHour(),
...
void CGuildLogDialog::OnBnClickedGuildLogStopButton()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
...
void CGuildLogDialog::OnBnClickedGuildLogExcelButton()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
...
BOOL CGuildLogDialog::OnInitDialog()
{
	...
	// 리스트 컨트롤 초기화
	{
		...
		// 아이템 목록을 채운다
		{
			...
			mResultListCtrl.InsertColumn( ++step, textRequest, LVCFMT_LEFT, int( rect.Width() * 0.25f ) );

			CString textTarget;
			textTarget.LoadString( IDS_STRING52 );
			mResultListCtrl.InsertColumn( ++step, textTarget, LVCFMT_LEFT, int( rect.Width() * 0.25f ) );

			CString textRank;
			textRank.LoadString( IDS_STRING43 );
			mResultListCtrl.InsertColumn( ++step, textRank, LVCFMT_LEFT, int( rect.Width() * 0.25f ) );
...
void CGuildLogDialog::Parse( DWORD serverIndex, const MSGROOT* message )
{
	...
	case MP_RM_GUILD_LOG_GET_ACK:
		{
			...
			for( DWORD i = 0; i < m->mSize; ++i )
			{
				...
				mResultListCtrl.SetItemText( row, ++step, mApplication.GetRankName( data.mValue2 ) );
			}

			{
				...
				mResultProgressCtrl.SetPos( mResultProgressCtrl.GetPos() + m->mSize );
...
INT_PTR CGuildLogDialog::DoModal( DWORD serverIndex, const CString& guildName, DWORD guildIndex )
{
	// TODO: 여기에 특수화된 코드를 추가 및/또는 기본 클래스를 호출합니다.
	mServerIndex	= serverIndex;
	mServerName		= mApplication.GetServerName( mServerIndex );
...
void CGuildLogDialog::OnBnClickedGuildLogClipboardButton()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
