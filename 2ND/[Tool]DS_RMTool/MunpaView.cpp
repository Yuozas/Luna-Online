...
void CMunpaView::OnInitialUpdate() 
{
	...
	// page link
	CGuildInfo*	pPage = new CGuildInfo;
	m_pPageList[eMunPage_GuildInfo] = pPage;	
	for( int i = 0; i < eMunPage_Max; ++i )
	{
...
void CMunpaView::SetGuildList( TMSG_GUILD* pMsg )
{
	CGuildInfo* pPage = (CGuildInfo*)m_pPageList[eMunPage_GuildInfo];
	if( pPage )
		pPage->SetGuildList( pMsg );	
}

void CMunpaView::SetGuildInfo( TMSG_GUILD* pMsg )
{
	CGuildInfo* pPage = (CGuildInfo*)m_pPageList[eMunPage_GuildInfo];
	if( pPage )
		pPage->SetGuildInfo( pMsg );	
}

void CMunpaView::SetGuildMemberList( TMSG_GUILDMEMBERINFO* pMsg )
{
	CGuildInfo* pPage = (CGuildInfo*)m_pPageList[eMunPage_GuildInfo];
	if( pPage )
		pPage->SetGuildMemberList( pMsg );
}

void CMunpaView::SetGuildWareItemInfo( TMSG_CHARACTER_ITEMBASE* pMsg )
{
	CGuildInfo* pPage = (CGuildInfo*)m_pPageList[eMunPage_GuildInfo];
	if( pPage )
		pPage->SetGuildWareItemInfo( pMsg );
}

void CMunpaView::SetGuildWareItemOptionInfo( TMSG_ITEMOPTIONINFO2* pMsg )
{
	CGuildInfo* pPage = (CGuildInfo*)m_pPageList[eMunPage_GuildInfo];
	if( pPage )
		pPage->SetGuildWareItemOptionInfo( pMsg );
}