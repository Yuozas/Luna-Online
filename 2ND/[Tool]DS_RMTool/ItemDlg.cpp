...
void CItemDlg::Init( INFOFIELD efield, const ITEMBASE* pitem, const ITEM_OPTION_INFO* pOption, WORD pos, WORD wWeartype )
{
	...
	if( pOption == NULL )
	{
		if( GAMERESRCMNGR->IsOptionItem(pitem->wIconIdx, (WORD)pitem->Durability) )
		//	ASSERT(0);
		m_bOption = FALSE;
	}
	else
...
void CItemDlg::SetItemTypeControl()
{
	...
	else if( m_Field == eIF_SHOPINVEN || m_Field == eIF_SHOPWARE )
	{
		EnableRadio(FALSE);
		SetItemCombo(0);
	}
...
void CItemDlg::SetItemCombo(DWORD type)
{
	...
	else
	{
		...
		while(pItemInfo = pItemInfoList->GetData())
		{
			if( pItemInfo->ItemKind == type )//|| pItemInfo->ItemKind == 16400 )
			{
