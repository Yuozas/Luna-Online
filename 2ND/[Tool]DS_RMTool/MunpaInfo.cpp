...
void CMunpaInfo::MakeToolTip(POSTYPE pos, WORD idx, char* str)
{
	...
	if( GAMERESRCMNGR->IsOptionItem(pInfo->ItemIdx, (WORD)m_pDocument->m_pWareHouse->WareHouse[pos].Durability) )
	{
		bOption = TRUE;
		OptionIdx = (WORD)m_pDocument->m_pWareHouse->WareHouse[pos].Durability;
						...
						ITEMBASE* pBItem, ITEMBASE* pAItem, ITEM_OPTION_INFO* pBOption, ITEM_OPTION_INFO* pAOption )
{
	...
	{ 
		...
		if( GAMERESRCMNGR->IsOptionItem(pBItem->wIconIdx, (WORD)pBItem->Durability) )
			m_pDocument->DelLocalOptionInfo((WORD)pBItem->Durability);
