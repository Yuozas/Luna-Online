...
enum eLogPage
{
	 eItemMoney_Page = 0, eTotal_Page, eEtc_Page, ePet_Page, eGuildTN_Page, eSiegeWar_Page, eGMTool_Page, ePageMax, eCharacter_Page, eSkill_Page, eAbility_Page,	 
};
...
public:
	...
	void	SetItemOptionLog( TMSG_ITEMOPTIONLOG* pMsg );
	void	SetPetLogCharacterIdx( DWORD dwCharIdx );
	void	SetLogPet( TMSG_LOGPET* pMsg );
	/// 060830 PKH 문파토너먼트
	void	setGuildTNLogGuildIdx( unsigned long guildidx );
	void	SetLogGuildTN( MSG_LOGGUILDTN* pMsg );
	/// 060831 PKH 공성전
	void	setSiegeWarLogGuildIdx( unsigned long guildidx );
	void	SetLogSiegeWar( MSG_LOGSIEGEWAR* pMsg );
	/// 060914 PKH GMTOOL
	void	SetGMToolCharacterIdx( DWORD dwCharIdx );
