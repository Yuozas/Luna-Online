...
void CLogDoc::NetworkMsgParse(BYTE Protocol, void* pMsg)
{
	...
	case MP_RMTOOL_QUERY_CHARIDX_ACK:
		{
			...
			else if( m_dwPage == ePet_Page)
				pView->SetPetLogCharacterIdx( pmsg->dwData );
			/// 060914 PKH GMTOOL
			else if( m_dwPage == eGMTool_Page)
				pView->SetGMToolCharacterIdx( pmsg->dwData );
		}
		break;
	...
	case MP_RMTOOL_HACKINGLOG_ACK:
		...
		break;
	case MP_RMTOOL_ITEMOPTIONLOG_ACK:
		{
			TMSG_ITEMOPTIONLOG* pmsg = (TMSG_ITEMOPTIONLOG*)pMsg;
			pView->SetItemOptionLog( pmsg );
		}
		break;
	case MP_RMTOOL_PETLOGFROMTYPE_ACK:
		{
			TMSG_LOGPET* pmsg = (TMSG_LOGPET*)pMsg;
			pView->SetLogPet( pmsg );
		}
		break;
	/// 060831 PKH 길드검색
	case MP_RMTOOL_QUERY_GUILDTNIDX_ACK:
		{
			TMSG_DWORD* pmsg = (TMSG_DWORD*)pMsg;
			pView->setGuildTNLogGuildIdx( pmsg->dwData );
		}
		break;
	/// 060831 PKH 문토로그결과
	case MP_RMTOOL_GUILDTNLOGFROMTYPE_ACK:
		{
			MSG_LOGGUILDTN* pmsg = (MSG_LOGGUILDTN*)pMsg;
			pView->SetLogGuildTN( pmsg );
		}
		break;
	/// 060831 PKH 공성전
	case MP_RMTOOL_QUERY_SIEGEWARIDX_ACK:
		{
			TMSG_DWORD* pmsg = (TMSG_DWORD*)pMsg;
			pView->setSiegeWarLogGuildIdx( pmsg->dwData );
		}
		break;
	/// 060831 PKH 공성전
	case MP_RMTOOL_SIEGEWARLOGFROMTYPE_ACK:
		{
			MSG_LOGSIEGEWAR* pmsg = (MSG_LOGSIEGEWAR*)pMsg;
			pView->SetLogSiegeWar( pmsg );
		}
		break;
	/// 060915 PKH GMTOOLLOG
	case MP_RMTOOL_GMTOOLLOGFROMTYPE_ACK:
		{
			MSG_LOGGMTOOL* pmsg = (MSG_LOGGMTOOL*)pMsg;
			pView->SetLogGMTool( pmsg );
		}
		break;
	}
}
