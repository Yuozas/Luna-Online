...
BEGIN_MESSAGE_MAP(CAdminInfo, CLDPropPage)
	...
	ON_BN_CLICKED(IDC_OPER_POWER_EVENTER, OnOperPowerEventer)
	ON_BN_CLICKED(IDC_OPER_POWER_QA, OnOperPowerQa)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
char* CAdminInfo::GetOperPowerName( WORD wPower, char* temp )
{
	...
	case eOperPower_Eventer:	sprintf( temp, "%s", "Eventer" );	break;		
	case eOperPower_QA:			sprintf( temp, "%s", "QA" );		break;
...
void CAdminInfo::OnOperPowerEventer() 
{
	m_nPower = eOperPower_Eventer;
}

void CAdminInfo::OnOperPowerQa() 
{	
	m_nPower = eOperPower_QA;
}
