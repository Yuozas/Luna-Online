...
char* CLogItemMoneyPage::GetLCData( LOGITEMMONEY* pData, DWORD dwIndex, char* temp )
{
	...
	case 3:	
		{
			if( pData->dwItemDBIdx == 0 )
			{
				sprintf( temp, "-");
				return temp;
			}
			else
				return GAMERESRCMNGR->GetItemMoneyLogType( pData->dwLogType, temp );
		}
		break;
...
char* CLogItemMoneyPage::GetLogType( DWORD dwLogType, char* temp )
{
	...
	case eLog_ItemMoveInvenToGuild:		sprintf( temp, "I ->> G" );	break;
	case eLog_ItemMoveGuildToInven:		sprintf( temp, "G ->> I" );	break;
	case eLog_ItemMoveGuildToGuild:		sprintf( temp, "G ->> G" );	break;
	//item obtain
...
//	case eLog_ItemMoveMunpaToMunpa:		sprintf( temp, "G ->> G" );	break;
	...
	case eLog_ItemObtainMonster:
		{
			MONSTEREX_LIST* pMonster = GAMERESRCMNGR->GetMonsterInfo( (WORD)dwIdx );
			if( pMonster == NULL )
				sprintf( temp, "???" );
			else
...
void CLogItemMoneyPage::InitComboViewerHow()
{
	...
	m_cbViewerHow.AddString( GetLogType( eLog_ItemMoveInvenToGuild, temp ) );
	m_cbViewerHow.SetItemData( 4, eLog_ItemMoveInvenToGuild );

	m_cbViewerHow.AddString( GetLogType( eLog_ItemMoveGuildToInven, temp ) );
	m_cbViewerHow.SetItemData( 5, eLog_ItemMoveGuildToInven );

	m_cbViewerHow.AddString( GetLogType( eLog_ItemMoveGuildToGuild, temp ) );
	m_cbViewerHow.SetItemData( 6, eLog_ItemMoveGuildToGuild );
...
void CLogItemMoneyPage::OnBtnSavetofile() 
{
	...
	while( pData = m_LogTable.GetData() )
	{
		fprintf( fp, "%d\t%s\t%d\t%s\t%s\t%d\t%s\t%d\t%d\t%s\t%d\t%d\n", pData->dwItemDBIdx, GetItemName( pData->dwItemIdx, temp1 ), pData->dwItemDur,
			GAMERESRCMNGR->GetItemMoneyLogType( pData->dwLogType, temp2 ), GetName( pData->dwLogType, pData->dwToChrIdx, pData->sToChrName ), pData->dwToTotalMoney,
...
void CLogItemMoneyPage::OnCheckTime() 
{
	if( m_bOneDay )
	{
		m_bTime = !m_bTime;	
