...
BEGIN_MESSAGE_MAP(CDS_RMToolApp, CWinApp)
	...
	ON_COMMAND(ID_FILE_NEW, (AFX_PMSG)OnFileNew)
...
BOOL CDS_RMToolApp::InitInstance()
{
	...
	pDocTemplate = new CMultiDocTemplate(
		...
		RUNTIME_CLASS(CPetFrm),
		RUNTIME_CLASS(CPetView));
	AddDocTemplate(pDocTemplate);

	// Delete Character Search temp
	pDocTemplate = new CMultiDocTemplate(
		IDR_DEL_CHAR,
		RUNTIME_CLASS(CDelCharDoc),
		RUNTIME_CLASS(CDelCharFrm),
		RUNTIME_CLASS(CDelCharView));
	AddDocTemplate(pDocTemplate);
