...
void CMunpaDoc::NetworkMsgParse(BYTE Protocol, void* pMsg)
{
	...
	switch(Protocol)
	{
	case MP_RMTOOL_GUILD_NACK:
	case MP_RMTOOL_GUILDIFNO_NACK:
		{
			MessageBox( NULL, "Not Guild!!", "Error", MB_OK );
		}
		break;
	case MP_RMTOOL_GUILD_ACK:
		{
			TMSG_GUILD* pmsg = (TMSG_GUILD*)pMsg;
			pView->SetGuildList( pmsg );			
		}
		break;
	case MP_RMTOOL_GUILDBASEIFNO:
		{
			TMSG_GUILD* pmsg = (TMSG_GUILD*)pMsg;
			pView->SetGuildInfo( pmsg );
		}
		break;
	case MP_RMTOOL_GUILDMEMBERIFNO:
		{
			TMSG_GUILDMEMBERINFO* pmsg = (TMSG_GUILDMEMBERINFO*)pMsg;
			pView->SetGuildMemberList( pmsg );
		}
		break;
	case MP_RMTOOL_GUILDITEMIFNO:
		{
			TMSG_CHARACTER_ITEMBASE* pmsg = (TMSG_CHARACTER_ITEMBASE*)pMsg;
			pView->SetGuildWareItemInfo( pmsg );
		}
		break;
	case MP_RMTOOL_GUILDITEMOPTIONIFNO:
		{
			TMSG_ITEMOPTIONINFO2* pmsg = (TMSG_ITEMOPTIONINFO2*)pMsg;
			pView->SetGuildWareItemOptionInfo( pmsg );
		}
		break;
