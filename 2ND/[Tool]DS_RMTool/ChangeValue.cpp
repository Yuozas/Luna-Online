...
DWORD CChangeItem::GetTargetIdx( CBaseDoc* pDoc )
{
	...
	else if( m_sInfo.TableKind == eTK_ShopWare )
		return ((CCharacterDoc*)pDoc)->m_sBaseInfo.dwUserID;
	else if( m_sInfo.TableKind == eTK_PetItem )
		return ((CPetDoc*)pDoc)->m_sBaseInfo.dwObjectID;
	else
		ASSERT(0);
...
char* CChangeItem::GetTargetName( CBaseDoc* pDoc )
{
	if( m_sInfo.TableKind == eTK_InventoryItem )
		...
		return ((CPetDoc*)pDoc)->m_sBaseInfo.ObjectName;
		
	else
	return NULL;	
}

BYTE CChangeItem::GetTableIdx()
{
	if( m_sInfo.TableKind == eTK_InventoryItem )
		return 0;
	else if( m_sInfo.TableKind == eTK_PyogukItem )
...
void CChangeItem::ApplyChangeValue( CBaseDoc* pDoc )
{
	...
	if(	m_sInfo.OperKind == eOp_Insert )
	{
		...
		{ // Update·Î ¹Ù²Û´Ù.
			...
			Msg.bDeleteOption = 0;
			if( GAMERESRCMNGR->IsOptionItem(m_BeforeItem.wIconIdx, (WORD)m_BeforeItem.Durability) && m_AfterOption.dwOptionIdx == 0 )
					Msg.bDeleteOption = 1;
