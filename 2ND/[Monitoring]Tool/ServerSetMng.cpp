...
void CServerSetMng::DoSendEventMessage( char* setname, char* pTitle, char* pMsg )
{
	if( pTitle == NULL || pMsg == NULL )	return;
	int len1 = strlen( pTitle );
	int len2 = strlen( pMsg );
...
void CServerSetMng::DoSendEventMessageOff( char* setname )
{
	if( strcmp( "All Server", setname ) == 0 )
	{
		for( int i = 0; i < m_nMaxServerSet; ++i )
...
void CServerSetMng::CheckAndSendSMS()
{
	...
	if( ret )
		CopyFile( "./Data/SMS_Msg.txt", "./SMS/SMSList.ini", FALSE );
}