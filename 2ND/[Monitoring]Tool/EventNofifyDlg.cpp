...
BOOL CEventNofifyDlg::OnInitDialog() 
{
	...
	m_strCurFileName = "None";
	m_bmpGreen.LoadBitmap( IDB_BITMAP_GREEN );
	m_editRepeat.SetWindowText( "1" );
	m_nRepeatNum = 1;
	m_editTime.SetWindowText( "60" );
	m_nRepeatTime = 60;	
	...
	if( m_bAutoNotify )
	{
		...
		if( m_nCheckTime <= 0 )
		{
			...
			if( m_NotifyMessage.IsRepeat() )
			{
				...
				SERVERSETMNG->DoSendEventMessage( (char*)(LPCTSTR)str, title, Msg );
					
				char temp[256];
				m_editRepeat.SetWindowText( itoa( m_NotifyMessage.GetRepeatNum(), temp, 10 ) );
...
void CEventNofifyDlg::OnButtonEventNotify() 
{
	if( m_bAutoNotify )
	{
		if( MessageBox( "Stop To Send Event Notification Message?", "Confirm!!", MB_OKCANCEL ) == IDOK )
		{
			m_btnNotify.SetBitmap( NULL );	
	...
	else
	{
		...
		{
			MessageBox( "First, You Have To Open Event Notification File!!", "Error!!" );
			return;
		}
		else
		{
			if( MessageBox( "Send Event Notification Message?", "Confirm!!", MB_OKCANCEL ) == IDOK )
			{
...
void CEventNofifyDlg::OnChangeEditEventMsg() 
{
	if( m_editMsg.m_bReturn )
	{
		...
		if( MessageBox( "Send Event Notification Message?", "Message", MB_OKCANCEL ) == IDOK )
		{
			m_cbServerSet.GetLBText( m_cbServerSet.GetCurSel(), setname );
			
			SERVERSETMNG->DoSendEventMessage( (char*)(LPCTSTR)setname, (char*)(LPCTSTR)title, (char*)(LPCTSTR)msg );
...
void CEventNofifyDlg::OnButtonOff() 
{
	...
	if( MessageBox( "Event Notification Message Off?", "Confirm!!", MB_OKCANCEL ) == IDOK )
	{
		CString setname;
		m_cbServerSet.GetLBText( m_cbServerSet.GetCurSel(), setname );
			
		SERVERSETMNG->DoSendEventMessageOff( (char*)(LPCTSTR)setname );
	}
}
